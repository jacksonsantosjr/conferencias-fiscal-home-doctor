"""
Adaptador de Parser para a Prefeitura do Rio de Janeiro.
Extrai notas fiscais do relatório oficial da Prefeitura do Rio de Janeiro (XLSX, CSV ou PDF),
com leitura resiliente por stream contra bloqueios de arquivo do Excel e limpeza automatizada de formatação de moeda ('R$ 6.587,74' -> 6587.74).
"""
...
class RioDeJaneiroParser(BaseCityParser):
    def __init__(self):
        super().__init__("Rio de Janeiro")
    ...
    def _parse_csv_bytes(self, csv_bytes: bytes) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for idx_row, row in enumerate(reader):
                ...
                if idx_cancelamento < len(row) and row[idx_cancelamento].strip():
                    continue

                raw_val = row[idx_valor].strip() if idx_valor < len(row) else ""
                val_str = raw_val.replace('R$', '').replace(' ', '').replace('\xa0', '').strip()
                if not val_str: continue

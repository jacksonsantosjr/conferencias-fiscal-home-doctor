"""
Adaptador de Parser para a Prefeitura de Belém (PA).
Extrai notas fiscais do relatório oficial da Prefeitura de Belém (XLSX, CSV ou PDF),
considerando estritamente a coluna 'Valor Serviços' (conforme premissa do usuário), com descarte de notas canceladas.
"""
...
class BelemParser(BaseCityParser):
    def __init__(self):
        super().__init__("Belém")
    ...
    def _parse_csv_bytes(self, csv_bytes: bytes) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for idx_row, row in enumerate(reader):
                ...
                num_cell = row[idx_numero].strip() if idx_numero < len(row) else f"BEL-{idx_row+1}"

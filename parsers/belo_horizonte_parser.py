"""
Adaptador de Parser para a Prefeitura de Belo Horizonte (MG).
Extrai notas fiscais do relatório oficial da Prefeitura de Belo Horizonte (XLSX, CSV ou PDF),
...
class BeloHorizonteParser(BaseCityParser):
    def __init__(self):
        super().__init__("Belo Horizonte")
    ...
    def _parse_csv_bytes(self, csv_bytes: bytes) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for idx_row, row in enumerate(reader):
                ...
                num_cell = row[idx_numero].strip() if idx_numero < len(row) else f"BH-{idx_row+1}"

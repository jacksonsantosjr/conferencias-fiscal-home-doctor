"""
Adaptador de Parser para a Prefeitura de Fortaleza (CE).
Extrai notas fiscais do relatório oficial da Prefeitura de Fortaleza (XLSX, CSV ou PDF),
com sanitização e conversão numérica resiliente (suportando texto '44401.39' e 'R$ 44.401,39') e descarte de notas canceladas.
"""
...
class FortalezaParser(BaseCityParser):
    def __init__(self):
        super().__init__("Fortaleza")
    ...
    def _parse_csv_bytes(self, csv_bytes: bytes) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for idx_row, row in enumerate(reader):
                ...
                num_cell = row[idx_numero].strip() if idx_numero < len(row) else f"FOR-{idx_row+1}"

"""
Adaptador de Parser para a Prefeitura de João Pessoa (PB).
Extrai notas fiscais do relatório oficial da Prefeitura de João Pessoa (XLSX, CSV ou PDF),
com sanitização e conversão numérica resiliente (suportando texto '25168.55' e 'R$ 25.168,55') e descarte de notas canceladas.
"""
...
class JoaoPessoaParser(BaseCityParser):
    def __init__(self):
        super().__init__("João Pessoa")
    ...
    def _parse_csv_bytes(self, csv_bytes: bytes) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for idx_row, row in enumerate(reader):
                ...
                num_cell = row[idx_numero].strip() if idx_numero < len(row) else f"JP-{idx_row+1}"

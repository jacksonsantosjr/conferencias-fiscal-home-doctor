    ...
    def _parse_xlsx_bytes(self, xlsx_bytes: bytes) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for row_idx in range(2, sheet.max_row+1):
                ...
                # Legacy escrituracao check
                if idx_escrituracao is not None:
    ...
    def _parse_csv_bytes(self, csv_bytes: bytes) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for idx, row in enumerate(rows_list):
                ...
                if 'valor dos servi' in row_str or 'nfs-e nacional' in row_str or 'escrituracao' in row_str:
                    headers = row
                    header_row_idx = idx
                    break
            ...
            for idx, h in enumerate(headers):
                ...
                if 'nfs-e nacional' in h_norm:
                    idx_numero = idx
                elif ('nota fiscal' in h_norm or 'nfs' in h_norm) and idx_numero is None:
                    idx_numero = idx
                elif 'valor' in h_norm:
                    idx_valor = idx
                elif 'iss retido' in h_norm:
                    idx_retido = idx
